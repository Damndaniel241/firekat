...
async function getTopics() {
  try {
    const response = await axios.get(
      "http://127.0.0.1:8000/api/subjects/19/topics/"
    );
...
<template>
  ...
  <button
    ...
    @click="goToNewPost(router, null, 19)"
  >
    (create new topic)
  </button>
  <RouterLink
  ...
  >
    ...
    >
      <RouterLink
        :to="{
          name: 'PostDetail',
          params: { topicId: topic.id, topicSlug: topic.slug },
        }"
      ...
      <div class="text-sm text-[#555518]">
        ...
        >. <span class="font-bold"> {{ topic.comment_count }} </span> posts
        <span class="font-bold"> {{ formatPostedAt(topic.posted_at) }}</span>
      </div>
    </div>
  </div>
  <Ads />
